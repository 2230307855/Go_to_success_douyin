...
- 点赞：https://xizew4izdpt.feishu.cn/docx/JgIFdS3nRoStY6xaij4cFSHkn9f#ElcYdJc6goDH98x7y6PcoDdrnFb
...
- 以项目初始状态进入抖音，项目没有视频，也无法进行点赞、关注、评论等，需要注册一个账号并登录
- 账号注册完毕后，会自动登录
  会有以下情况：
  - （1）用户重复：登录失败，并提示用户存在
  - （2）用户不存在：提示不存在用户
  - （3）密码错误：提示密码错误
  - （4）权限验证失败：提示权限已过期或不合法

- 发布视频，点击下方的加号，即可进入本地的静态视频的选择，添加标题后可进行发布操作，发布成功会有，“视频上传成功”的提示
- feed流，在首页下拉即可刷新页面，此时会将最新发布的视频进行展示，视频可无限滚动
- 发布列表，首页底栏点击“我”，即可看到自己已经发布的视频，没有发布视频列表为空
- 点击发布视频的作者的头像，即可进入该作者的详情页面，可以查看该作者的详情信息
- 点击首页视频的红心，红心会变成红色，且红心下会有红心数量的统计，再次点击会重新变为白色，且红心总数减1
- 点击首页的消息按钮，会先弹出一个对话框，展示此视频的历史评论信息，如果此时进行评论输入并点击发送按钮，则会将最新的评论置顶，没有输入点击发送按钮不会有新的评论生成
- 在首页点击作者的头像在详情页点击关注即可关注此作者，红色的关注按钮也会变成白色，再次点击白色按钮，又会回到原来的状态。同样首页中可点击该作者头像下的“+“来关注作者，关注之后”+”会消失，当取关后优惠显示出“+”号。关注成功自己主页的关注数也会加1，且被关注者的粉丝数会加1
- 在有了关注用户后底栏点击“我”后的详情页面会显示自己的关注数，点击关注数，会看到自己的关注列表和粉丝列表，在列表的每一项中可以对已关注着进行取关或再次关注，在粉丝列表中每一项可以选择将其关注
- 好友，用户双方互相关注即可成为好友
- 聊天，为好友关系的两个用户可在，底栏中点击消息查看和回复好友的信息，信息也会实时更新

## 5.2 演示视频

https://xizew4izdpt.feishu.cn/docx/JgIFdS3nRoStY6xaij4cFSHkn9f#MkB7dj7vqo7v72xmjEHcjpixnhc
